...
requirements/main.txt: requirements/main.in
	...
	pip-compile requurements/main.in

requirements/dev.txt: requirements/dev.in
	pip install --upgrade pip
	pip install --upgrade pip-tools
	pip-compile requurements/dev.in
...
coverage:
	coverage run --source nlsummary_utils setup.py test
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs:
	rm -f docs/nlsummary_utils.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ nlsummary_utils
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
